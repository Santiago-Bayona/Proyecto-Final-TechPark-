...
public interface HistorialVisitaRepository extends JpaRepository<HistorialVisita, Long> {
    ...
    @Query("DELETE FROM HistorialVisita h WHERE h.atraccion.id = :atraccionId")
    void deleteByAtraccionId(@org.springframework.data.repository.query.Param("atraccionId") Long atraccionId);
}